...
const LoginPage = () => {
    ...
    const handleSubmit = async (event: SyntheticEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (remember) {
                localStorage.setItem("goldensweep_session", JSON.stringify(demoSession))
            } else {
                sessionStorage.setItem("goldensweep_session", JSON.stringify(demoSession))
            }
    ...
    return (
        ...
        >
            <form onSubmit={handleSubmit} className="space-y-5" noValidate>
                ...
                <div>
                    <label htmlFor="email" className="text-xs font-black uppercase tracking-[0.12em] text-white/60">
                        Email Address
                    </label>

                    <div className={`mt-2 flex h-[54px] items-center rounded-xl border bg-black/25 px-4 transition ${errors.email ? "border-red-400/50" : "border-white/[0.09] focus-within:border-gold-400/45"
                        }`}>
                        <Mail size={19} className="shrink-0 text-gold-400/80" />
                ...
                <div>
                    <div className="flex items-center justify-between">
                        <label htmlFor="password" className="text-xs font-black uppercase tracking-[0.12em] text-white/60">
                            Password
                        </label>

                        <Link to="/forgot-password" className="text-xs font-bold text-gold-300 transition hover:text-gold-200">
                            Forgot password?
                        </Link>
                    </div>

                    <div className={`mt-2 flex h-[54px] items-center rounded-xl border bg-black/25 px-4 transition ${errors.password ? "border-red-400/50" : "border-white/[0.09] focus-within:border-gold-400/45"
                        }`}>
                        <LockKeyhole size={19} className="shrink-0 text-gold-400/80" />
                ...
                <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-1.5 text-[11px] text-white/30">
                        <ShieldCheck size={14} className="text-gold-400/70" />
                        Secure sign in
                    </div>
                </div>
                ...
                <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] px-4 py-3">
                    <p className="text-xs leading-5 text-white/35">
                        By signing in, you agree to GoldenSweep's{" "}
